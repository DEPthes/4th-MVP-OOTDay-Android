...
interface AuthApiService {
    ...
    suspend fun requestPhoneNumberAuth(@Body request: PhoneNumberAuthRequest): Response<PhoneNumberAuthResponse>

    @POST("api/auth/check-phone-auth") // 실제 백엔드 API 엔드포인트로 변경하세요.
    suspend fun checkPhoneNumberAuth(@Body request: AuthCheckRequest): Response<AuthCheckResponse>
}